...
// When set, an inbound call's outbound dial (transfer) leg is sent back to the
// SBC the call arrived on (static-IP feature). In split-SBC deployments the
// ingress SBC (sbc-inbound) can't route outbound, so this must stay OFF — the
// dial leg then uses getSBC() (the outbound SBC pool), the same path
// originations already use. (consigai/jambonz-feature-server #777)
const JAMBONES_DIAL_REUSE_INGRESS_SBC = process.env.JAMBONES_DIAL_REUSE_INGRESS_SBC || false;
...
module.exports = {
  ...
  JAMBONES_DIAL_REUSE_INGRESS_SBC,
