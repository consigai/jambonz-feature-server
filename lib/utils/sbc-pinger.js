...
const {
  ...
  NODE_ENV,
  JAMBONES_CLUSTER_ID,
  SBC_MAX_FAILED_PINGS,
  SBC_HEALTH_CHECK_ENABLED,
  SBC_PING_TIMEOUT_MS,
} = require('../config');
...
module.exports = (logger) => {
  ...
  // Initialize external health monitor
  const healthMonitor = new SbcHealthMonitor(logger, {
    enabled: SBC_HEALTH_CHECK_ENABLED,
    maxFailedPings: parseInt(SBC_MAX_FAILED_PINGS),
    timeoutMs: parseInt(SBC_PING_TIMEOUT_MS),
    clusterId: JAMBONES_CLUSTER_ID || 'default',
    preventLastSbcRemoval: true // Safety first
  });
  ...
  async function pingProxies(srf) {
    ...
    for (const sbc of sbcs) {
      try {
        ...
        // Set up health monitoring for this ping
        healthMonitor.beforePing(sbc);
        
        req.on('response', (res) => {
          debug(`received ${res.status} from SBC ${sbc}`);
    ...
    // Process any SBCs that need to be removed due to health failures
    const sbcsToRemove = healthMonitor.getSbcsToRemove();
    if (sbcsToRemove.length > 0) {
      const {removeFromSet} = srf.locals.dbHelpers;
      for (const sbc of sbcsToRemove) {
        healthMonitor.removeUnhealthySbc(sbc, sbcs, removeFromSet)
          .catch((err) => logger.error({err}, 'Error removing unhealthy SBC'));
      }
    }
  }
  ...
  return {
    ...
    healthMonitor, // Expose for debugging/monitoring
    cleanup: () => healthMonitor.cleanup()
  };
};
