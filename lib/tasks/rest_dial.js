...
class TaskRestDial extends Task {
  ...
  */
  ...
  async _onEarlyMedia() {
    ...
    this.logger.info('TaskRestDial:_onEarlyMedia - treating early media as connected call');
    ...
    try {
      ...
      if (tasks && Array.isArray(tasks)) {
        this.logger.debug({tasks: tasks}, `TaskRestDial: replacing application with ${tasks.length} tasks in early media`);
        cs.replaceApplication(normalizeJambones(this.logger, tasks).map((tdata) => makeTask(this.logger, tdata)));
      }
    } catch (err) {
