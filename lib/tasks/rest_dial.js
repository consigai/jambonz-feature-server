...
class TaskRestDial extends Task {
  constructor(logger, opts) {
    ...
    this.on('finalAnswer', this._onFinalAnswer.bind(this));
  }
  ...
  */
  ...
  _onFinalAnswer(finalDlg) {
    const cs = this.callSession;
    this.logger.info({ 
      callSid: cs ? cs.callSid : 'No CS',
      dialogId: finalDlg ? finalDlg.id : 'No finalDlg',
      earlyMediaConnected: this.earlyMediaConnected,
      csEarlyMediaConnected: cs ? cs.earlyMediaConnected : 'No CS'
    }, 'TaskRestDial:_onFinalAnswer - Received final answer');

    if (cs && this.earlyMediaConnected) {
      this.logger.info({ callSid: cs.callSid }, 'TaskRestDial:_onFinalAnswer - Updating dialog on CallSession');
      cs.updateDialog(finalDlg); 
    } else {
      this.logger.warn({ 
        callSid: cs ? cs.callSid : 'No CS', 
        earlyMediaConnected: this.earlyMediaConnected,
        csEarlyMediaConnected: cs ? cs.earlyMediaConnected : 'No CS'
      }, 'TaskRestDial:_onFinalAnswer - Not in early media state or cs missing. This might be unexpected. Falling back to _onConnect.');
      // Fallback: if we weren't actually in early media mode (e.g. flag was false), 
      // treat it like a normal connect. This path should ideally not be hit if call flow is correct.
      this._onConnect(finalDlg);
    }
    // Note: The _onConnect method handles canCancel = false and clearing call timer if it runs.
    // If only updateDialog runs, those are already handled by the early media flow and the 200 OK itself.
    this.canCancel = false; // Ensure canCancel is false as we have a final answer
    this._clearCallTimer(); // Clear any timeout timer as call is now fully established
  }
