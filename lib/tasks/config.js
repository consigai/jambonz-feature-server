...
class TaskConfig extends Task {
  ...
  async exec(cs, {ep} = {}) {
    ...
    // You pass in an action hook url to receive events on and a sticky bool attribute
    // if you want us to restart the beep detection after each event.
    if (this.data.beep) {
      this.logger.debug('Enabling Background Beep Events');
      const {startBeep, stopBeep} = require('../utils/beep-utils')(this.logger);
      this.startBeep = startBeep;
      this.stopBeep  = stopBeep;
      this.beepOptions = this.data.beep.options;
      this.on('beep', this._onBeepEvent.bind(this, cs));

      try {
        this.ep = ep;
        this.startBeep(cs, ep, this, this.beepOptions);
      } catch (err) {
  ...
  async kill(cs) {
    ...
    if (this.ep && this.stopBeep) this.stopBeep(this.ep, this);
  }

  _onBeepEvent(cs, evt) {
    this.logger.info({evt}, 'Config:_onBeepEvent');
    const {actionHook, sticky} = this.data.beep;
    this.performHook(cs, actionHook, evt)
      .catch((err) => {
        this.logger.error({err}, 'Config:_onBeepEvent - error calling actionHook');
      });
    if (sticky) {
      this.logger.info('Config:_onBeepEvent - sticky is true');
      this.startBeep(cs, this.ep, this, this.beepOptions);
    } else {
      this.logger.info('Config:_onBeepEvent - sticky is false');
    }
