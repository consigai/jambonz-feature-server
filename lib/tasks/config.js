...
class TaskConfig extends Task {
  constructor(logger, opts) {
    ...
    // setup our beep object to store things in
    this.beep = {};
    this.beep.lastEvent = 0;
  }
  ...
  get summary() {
    ...
    if (this.data.beep) phrase.push('enable beep');
  ...
  async exec(cs, {ep} = {}) {
    ...
    // Support for receiving beep events in the background over the action hook.
    if (this.data.beep) {
      const beepConfig = this.data.beep;
      const isEnabled = beepConfig.enable === undefined || beepConfig.enable === true;

      if (isEnabled) {
        if (!beepConfig.actionHook) {
          this.logger.error('Config:exec - Beep enabled but no actionHook provided.');
        } else {
          this.logger.debug('Enabling Background Beep Events');
          const {startBeep, stopBeep: stopBeepUtil} = require('../utils/beep-utils')(this.logger);
          this.beep.start = startBeep;
          this.beep.stop = stopBeepUtil; // Store for potential future use if task itself needs to stop its own started beep
          this.on('beep', this._onBeepEvent.bind(this, cs));

          try {
            this.ep = ep;
            this.beep.start(cs, ep, this);
          } catch (err) {
            this.logger.info({err}, 'Config:exec - Error calling startBeep');
          }
        }
      } else { // beepConfig.enable === false
        this.logger.debug('Disabling Background Beep Events due to enable:false');
        const {stopBeep} = require('../utils/beep-utils')(this.logger);
        try {
          stopBeep(cs, ep);
        } catch (err) {
          this.logger.error({err}, 'Config:exec - Error calling stopBeep utility for enable:false');
        }
      }
    } // If this.data.beep is not present, do nothing to the current beep state.

    if (this.data.amd) {
      this.logger.debug('Enabling Background AMD Events');
  ...
  async kill(cs) {
    ...
    // Beep stopping is now handled by explicit enable:false, not task kill.
  }

  _onBeepEvent(cs, evt) {
    this.logger.info({evt}, 'Config:_onBeepEvent');
    const {actionHook} = this.data.beep;
    this.performHook(cs, actionHook, evt)
      .catch((err) => {
        this.logger.error({err}, 'Config:_onBeepEvent - error calling actionHook');
      });
  }
