...
const {ANCHOR_MEDIA_ALWAYS,
  ...
  JAMBONES_DIAL_REUSE_INGRESS_SBC} = require('../config');
...
class TaskDial extends Task {
  ...
  async _initializeInbound(cs) {
    ...
    /* Optionally send outbound legs back to the ingress SBC (static-IP feature).
     * Off by default: in split-SBC deployments the ingress SBC (sbc-inbound)
     * has no outbound routing, so reusing it makes the transfer/dial B-leg die
     * with a fast 480. Leaving this.proxy unset lets _attemptCalls fall to
     * getSBC() (the outbound SBC pool) — the same path originations use.
     * (consigai/jambonz-feature-server #777) */
    if (!this.proxy && JAMBONES_DIAL_REUSE_INGRESS_SBC) {
