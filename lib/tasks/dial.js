...
const {ANCHOR_MEDIA_ALWAYS,
  ...
  JAMBONES_DIAL_USE_OUTBOUND_SBC} = require('../config');
...
class TaskDial extends Task {
  ...
  async _initializeInbound(cs) {
    ...
    /* Reuse the ingress SBC for the outbound dial/transfer leg (static-IP
     * feature, upstream default). In split-SBC deployments the ingress SBC is
     * sbc-inbound, which has no outbound routing, so reusing it makes the
     * dial/transfer B-leg die with a fast 480. Set JAMBONES_DIAL_USE_OUTBOUND_SBC
     * there: this.proxy stays unset and _attemptCalls falls to getSBC() (the
     * outbound pool), the same path originations use. (consigai/jambonz-feature-server #777) */
    if (!this.proxy && !JAMBONES_DIAL_USE_OUTBOUND_SBC) {
      this.proxy = `${cs.req.source_address}:${cs.req.source_port}`;
    }
