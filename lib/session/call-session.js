...
Duration=${duration} `
 ...
 got response to INFO DTMF digit=${digit} and duration=${duration}`);
  ...
  async _notifyCallStatusChange({callStatus, sipStatus, sipReason, duration}) {
    ...
    // Add detailed logging for tracking status transitions
    this.logger.info({
      ...
      sipReason,
      duration,
      earlyMediaConnected: this.isRestCallSession && this.earlyMediaConnected,
      hasDialog: !!this.dlg
    }, `*** CALL STATUS CHANGE: ${callStatus} (SIP ${sipStatus}) ***`);
  ...
  async executeStatusCallback(callStatus, sipStatus) {
    ...
    this.logger.info({
      ...
      callStatus,
      sipStatus,
      webhookUrl: this.call_status_hook?.url
    }, `*** SENDING STATUS WEBHOOK: ${callStatus} to ${this.call_status_hook?.url} ***`);
    ...
    try {
      ...
      this.logger.debug({
        callSid: this.callSid,
        callStatus
      }, 'CallSession:executeStatusCallback - webhook successfully sent');
      ...
      this.logger.info({
        ...
        callStatus,
        sipStatus
      }, `CallSession:executeStatusCallback - ERROR sending webhook for ${callStatus}`);
    }
  }
