...
class RestCallSession extends CallSession {
  ...
  setDialog(dlg) {
    ...
    if (this.dlg) {
      this.logger.debug('RestCallSession:setDialog - transitioning from early media to connected call');
  ...
  updateDialog(dlg) {
    ...
    this.logger.info('RestCallSession: dialog updated from early media to connected call');
  }
