     ...
     */
      ...
      try {
        ...
        if (!cs.earlyMediaConnected) {
          ...
          // We were previously connected in early media - just update the dialog
          // Don't emit another connect event since we already processed it
          sipLogger.info({
            callSid: cs.callSid
          }, '*** EARLY MEDIA TO CONNECTED TRANSITION - ONLY UPDATING DIALOG ***');
          restDial.emit('callStatus', 200);
        }
      }
      catch (err) {
