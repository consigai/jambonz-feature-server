  ...
  async(req, res) => {
     ...
     */
      ...
      try {
          ...
          cbProvisional: (prov) => {
            ...
            if ([180, 183].includes(prov.status) && prov.body) {
              sipLogger.info({
                ...
                callStatus,
                hasSdp: !!prov.body,
                isEarlyMediaEligible: callStatus === CallStatus.EarlyMedia,
              }, '*** GOT PROVISIONAL RESPONSE WITH SDP - EARLY MEDIA ELIGIBLE ***');
            ...
            // Emit earlyMedia event when we get 183 with SDP
            if (callStatus === CallStatus.EarlyMedia) {
              sipLogger.info({
                callSid: cs.callSid,
                sipStatus: prov.status,
                provDialogDefined: !!prov.dialog,
                provDialogId: prov.dialog ? prov.dialog.id : 'prov.dialog is null/undefined',
                csReqDefined: !!cs.req,
                csReqDialogDefined: !!(cs.req && cs.req.dialog),
                csReqDialogId: cs.req && cs.req.dialog ? cs.req.dialog.id : 'cs.req.dialog is null/undefined'
              }, '*** EMITTING EARLY MEDIA EVENT ***');
              // Pass the dialog associated with the CallSession's initial INVITE request
              // This dialog should be updated by drachtio-srf upon receiving the provisional with SDP
              restDial.emit('earlyMedia', cs.req ? cs.req.dialog : null);
            }
          }
        });
        connectStream(dlg.remote.sdp);
        
        // Always emit callStatusChange for the 200 OK
        sipLogger.info({
          callSid: cs.callSid,
          wasEarlyMediaConnected: cs.earlyMediaConnected,
          receivedSipSuccess: true
        }, '*** RECEIVED 200 OK - CALL CONNECTED ***');
        ...
        // If not already connected in early media, emit the regular connect event
        if (!cs.earlyMediaConnected) {
          sipLogger.info({
            callSid: cs.callSid
          }, '*** NORMAL CONNECT FLOW - NO EARLY MEDIA WAS USED ***');
          restDial.emit('callStatus', 200);
          restDial.emit('connect', dlg);
        } else {
          // We were previously connected in early media - notify TaskRestDial with the final dialog
          sipLogger.info({
            callSid: cs.callSid,
            finalDialogId: dlg ? dlg.id : 'N/A'
          }, '*** EARLY MEDIA TO CONNECTED TRANSITION - EMITTING finalAnswer WITH NEW DIALOG ***');
          restDial.emit('callStatus', 200); // Still useful for general status
          restDial.emit('finalAnswer', dlg); // Pass the final dialog object
        }
      }
      catch (err) {
        ...
        if (err instanceof SipError) {
          ...
          // If we were in early media mode, we need to notify the application that the call failed
          if (cs && cs.earlyMediaConnected) {
            sipLogger.info({
              callSid: cs.callSid,
              sipStatus: err.status,
              sipReason: err.reason
            }, '*** CALL FAILED AFTER EARLY MEDIA CONNECTION - EXTRA CLEANUP REQUIRED ***');
          }
        }
        else {
          if (cs) cs.emit('callStatusChange', {
          ...
          if (sipLogger) sipLogger.error({
            err, 
            earlyMediaConnected: cs?.earlyMediaConnected,
            callSid: cs?.callSid
          }, '*** REST outdial failed with internal error ***');
