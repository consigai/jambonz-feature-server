...
router.post('/',
  ...
  async(req, res) => {
    ...
    try {
      ...
      // do not pass non-rest:dial fields to the verb engine
      const { transcribe: _tx, recordEarlyMedia: _rem, ...restDialBody } = req.body;
      const restDial = makeTask(logger, { 'rest:dial': restDialBody });
     ...
     *  */
      ...
      /* launch outdial */
      let sdp, sipLogger;
      let earlyFeaturesStarted = false;
      let earlyMediaTranscribeTask = null;
      let earlyMediaTranscribeOnStatus = null;
      let dualEp;
      ...
      const connectStream = async(remoteSdp) => {
        if (remoteSdp !== sdp) {
          ...
          // If call is now answered and we have an active early-media transcription task, kill it
          // This handles the case where connectStream is called again after the call answers (200 OK)
          // Note: cs.dlg may not be set yet if called from cbProvisional, but cs.callInfo.sipStatus should be updated
          const callNowAnswered = cs.dlg || (cs.callInfo && cs.callInfo.sipStatus === 200);
          if (callNowAnswered && earlyMediaTranscribeTask && earlyFeaturesStarted) {
            logger.debug({callSid: cs.callSid},
              'early-media transcribe: call answered, killing active early-media transcription task');
            try {
              earlyMediaTranscribeTask.kill(cs);
              if (earlyMediaTranscribeOnStatus) {
                cs.off('callStatusChange', earlyMediaTranscribeOnStatus);
                earlyMediaTranscribeOnStatus = null;
              }
              earlyMediaTranscribeTask = null;
              logger.debug({callSid: cs.callSid},
                'early-media transcribe: task killed on call answer');
            } catch (e) {
              logger.info({err: e, callSid: cs.callSid},
                'early-media transcribe: error killing task on call answer');
            }
          }
          // initialize early-media features once when SDP first connects
          if (!earlyFeaturesStarted) {
            earlyFeaturesStarted = true;
            try {
              // Early-media transcription (default enabled if transcribe provided)
              const tx = req.body.transcribe;
              if (tx && (tx.earlyMedia === true || typeof tx.earlyMedia === 'undefined')) {
                if (typeof tx !== 'object' || Array.isArray(tx)) {
                  logger.info({tx}, 'early-media: transcribe payload is not an object, skipping');
                } else {
                  // remove non-verb control fields before creating the task
                  const { stopOnAnswer, earlyMedia, ...txVerb } = tx;
                  const task = makeTask(logger, { transcribe: txVerb });
                  earlyMediaTranscribeTask = task;
                  const resources = await cs._evaluatePreconditions(task);
                  // attach tracing context for the early-media task
                  const {span, ctx} = cs.rootSpan.startChildSpan(`earlymedia-transcribe:${task.summary}`);
                  task.span = span;
                  task.ctx = ctx;

                  // Stop STT on 200 OK by default unless stopOnAnswer is explicitly false
                  // Register listener BEFORE executing task to avoid race condition where
                  // call is already answered (200 OK) before listener is registered
                  const shouldStopOnAnswer = stopOnAnswer !== false;
                  let callAlreadyAnswered = false;
                  if (shouldStopOnAnswer) {
                    logger.debug({callSid: cs.callSid},
                      'early-media transcribe: registering callStatusChange listener (stopOnAnswer enabled)');
                    earlyMediaTranscribeOnStatus = ({sipStatus}) => {
                      if (sipStatus === 200) {
                        logger.debug({callSid: cs.callSid},
                          'early-media transcribe: call answered (200 OK), killing transcribe task');
                        try {
                          if (earlyMediaTranscribeTask) {
                            earlyMediaTranscribeTask.kill(cs);
                            logger.debug({callSid: cs.callSid},
                              'early-media transcribe: task killed successfully');
                          }
                        } catch (e) {
                          logger.info({err: e, callSid: cs.callSid},
                            'early-media transcribe: error killing task on call answer');
                    ...
                    cs.on('callStatusChange', earlyMediaTranscribeOnStatus);
                    // Check if call is already answered (race condition fix)
                    // For outbound calls, dlg exists when call is answered
                    callAlreadyAnswered = cs.dlg ||
                      (cs.callInfo && cs.callInfo.sipStatus === 200);
                    if (callAlreadyAnswered) {
                      logger.info({
                        callSid: cs.callSid,
                        hasDlg: !!cs.dlg,
                        callInfoSipStatus: cs.callInfo?.sipStatus
                      },
                      'early-media transcribe: call already answered when setting up listener, skipping execution');
                      try {
                        if (earlyMediaTranscribeTask) {
                          earlyMediaTranscribeTask.kill(cs);
                        }
                        ...
                        span.end();
                        logger.debug({callSid: cs.callSid},
                          'early-media transcribe: task killed, listener removed (call already answered)');
                      } catch (e) {
                        logger.info({err: e, callSid: cs.callSid},
                          'early-media transcribe: error killing task (call already answered)');
                      }
                      earlyMediaTranscribeTask = null;
                    } else {
                      logger.debug({callSid: cs.callSid},
                        'early-media transcribe: call not yet answered, will execute task and wait for answer event');
                    }
                  } else {
                    logger.debug({callSid: cs.callSid},
                      'early-media transcribe: stopOnAnswer disabled, task will continue after call answer');
                  }

                  // Only execute task if call is not already answered
                  if (!callAlreadyAnswered) {
                    logger.debug({callSid: cs.callSid}, 'early-media transcribe: executing task');
                    task.exec(cs, resources)
                      .then(() => {
                        try { span.end(); } catch (e) {}
                        logger.debug({callSid: cs.callSid},
                          'early-media transcribe: task execution completed');
                        return null;
                      })
                      .catch((err) => logger.info({err, callSid: cs.callSid},
                        'early-media transcribe task error'));
                  } else {
                    logger.debug({callSid: cs.callSid},
                      'early-media transcribe: task execution skipped (call already answered)');
                  }
                }
              }

              // Early-media recording: strictly gated by recordEarlyMedia flag
              if (req.body.recordEarlyMedia === true) {
                try {
                  // force background record task to start immediately (even if account flag is off)
                  cs.backgroundTaskManager.newTask('record', {force: true});
                } catch (err) {
                  logger.info({err}, 'error starting early-media recording');
                }
              }
            } catch (err) {
              logger.info({err}, 'error initializing early-media features');
            }
          }
          return true;
        }
     ...
     */
      if ('WS' === app.call_hook?.method || /^wss?:/.test(app.call_hook.url)) {
        ...
        // ensure top-level call_sid is available immediately for early-media hooks
        try { app.requestor.call_sid = callSid; } catch (e) {}
      ...
      // if notifier is a websocket too (when re-used), make sure it has call_sid set as well
      try { if (app.notifier && app.notifier.call_sid === undefined) app.notifier.call_sid = callSid; } catch (e) {}
      ...
      try {
        ...
        // Ensure early-media transcription is killed when call answers
        if (earlyMediaTranscribeTask) {
          logger.debug({callSid: cs.callSid},
            'early-media transcribe: call answered (200 OK), killing active early-media transcription task');
          try {
            earlyMediaTranscribeTask.kill(cs);
            if (earlyMediaTranscribeOnStatus) {
              cs.off('callStatusChange', earlyMediaTranscribeOnStatus);
            ...
            earlyMediaTranscribeTask = null;
            logger.debug({callSid: cs.callSid},
              'early-media transcribe: task killed on call answer (200 OK)');
          } catch (e) {
            logger.info({err: e, callSid: cs.callSid},
              'early-media transcribe: error killing task on call answer (200 OK)');
          }
        }
